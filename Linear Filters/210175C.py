...
image_path = r"E:\UOM\Image Processing\Quiz\road75.png"
...
def apply_filter(image, filter_matrix, filter_name):
    ...
    # Pad the image with black pixels (zero padding)
    padded_image = np.pad(image, padding_size, mode='constant', constant_values=0)
    
    # Prepare the output image (same size as the input image)
    output_image = np.zeros_like(image)
    
    # Apply the filter to the image
    for i in range(image_height):
        for j in range(image_width):
            # Extract the region of the padded image that corresponds to the filter
            region = padded_image[i:i+filter_size, j:j+filter_size]
            
            # Apply the filter (element-wise multiplication followed by sum)
